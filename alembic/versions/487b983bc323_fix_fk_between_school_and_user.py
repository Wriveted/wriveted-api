...
def downgrade():
    ...
    op.drop_constraint('fk_admin_school', 'users', type_='foreignkey')
    op.drop_column('users', 'school_id_as_admin')
    op.add_column('users', sa.Column('school_id_as_admin', sa.INTEGER(), autoincrement=False, nullable=True))
    op.create_foreign_key('fk_admin_school', 'users', 'schools', ['school_id_as_admin'], ['id'])
    op.drop_constraint(None, 'schools', type_='foreignkey')
    op.drop_column('schools', 'admin_id')
    op.add_column('schools', sa.Column('admin_id', postgresql.UUID(as_uuid=True), autoincrement=False, nullable=True))
