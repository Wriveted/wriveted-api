...
class BookList(Base):
    ...
    school_id = Column(ForeignKey('schools.id', name="fk_booklist_school"), nullable=True)
    school = relationship("School", back_populates='booklists', foreign_keys=[school_id])
    ...
    service_account_id = Column(ForeignKey('service_accounts.id', name="fk_booklist_service_account"), nullable=True)
    service_account = relationship("ServiceAccount", back_populates='booklists', foreign_keys=[service_account_id])
