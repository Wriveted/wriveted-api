...
def validate_image_url_or_base64_string(
    ...
    def _validate_image_url_or_base64_string(cls, v, values: dict):
        if not v:
            return

        logger.info(f"Validating {field_name} `{v[0:100]}...`")
        ...
        if img.format.lower() not in ["jpg", "jpeg", "png"]:
            raise ValueError(
                f"{field_name} must be either `jpg`, `jpeg`, or `png` format"
            )
    ...
    return _validate_image_url_or_base64_string
