...
async def cleanup_test_data(async_session: AsyncSession):
        ...
        try:
            ...
            for work_id in work_ids:
                ...
                await async_session.execute(
                    text("DELETE FROM author_work_association WHERE work_id = :work_id"),
                    {"work_id": work_id}
                )
                await async_session.execute(
                    text("DELETE FROM series_works_association WHERE work_id = :work_id"),
                    {"work_id": work_id}
                )
                await async_session.execute(
                ...
                await async_session.execute(
                    text("DELETE FROM works WHERE id = :work_id"),
                    {"work_id": work_id}
                )
    ...
    for author_name in test_author_names:
        try:
            ...
            result = await async_session.execute(
                text("SELECT id FROM authors WHERE first_name = :first_name AND last_name = :last_name"),
            ...
            for author_id in author_ids:
                await async_session.execute(
                    text("DELETE FROM author_work_association WHERE author_id = :author_id"),
                    {"author_id": author_id}
                )
                await async_session.execute(
    ...
    for series_title in test_series_names:
        try:
            ...
            for series_id in series_ids:
                await async_session.execute(
                    text("DELETE FROM series_works_association WHERE series_id = :series_id"),
                    {"series_id": series_id}
                )
                await async_session.execute(
...
class TestSearchViewV1MaterializedView:
    ...
    async def test_search_view_refresh_after_work_creation(
        ...
        initial_result = await async_session.execute(
            text("SELECT COUNT(*) FROM search_view_v1 WHERE work_id IN (SELECT id FROM works WHERE title = :title)"),
            {"title": test_title}
        )
        ...
        assert pre_refresh_count == 0, "Work should not be in materialized view before refresh"
        ...
        post_refresh_result = await async_session.execute(
            text("SELECT work_id, author_ids FROM search_view_v1 WHERE work_id = :work_id"),
            {"work_id": new_work.id}
        )
    ...
    async def test_search_view_basic_structure(
        ...
        view_result = await async_session.execute(
            text("SELECT work_id, author_ids, series_id FROM search_view_v1 WHERE work_id = :work_id"),
            {"work_id": work.id}
        )
    ...
    async def test_search_view_staleness_without_refresh(
        ...
        # Create new work without refreshing view
        ...
        # Check that view count hasn't changed (stale data)
        stale_result = await async_session.execute(
            text("SELECT COUNT(*) FROM search_view_v1")
        )
        stale_count = stale_result.scalar()
        assert stale_count == initial_count, "Materialized view should be stale without refresh"
        
        # Verify the specific work is not in the view
        work_search_result = await async_session.execute(
            text("SELECT COUNT(*) FROM search_view_v1 WHERE work_id = :work_id"),
            {"work_id": new_work.id}
        )
        work_count = work_search_result.scalar()
        assert work_count == 0, "New work should not be in stale materialized view"
        
        # Now refresh and verify it appears
        await async_session.execute(text("REFRESH MATERIALIZED VIEW search_view_v1"))
        await async_session.commit()
        
        fresh_result = await async_session.execute(
            text("SELECT COUNT(*) FROM search_view_v1")
        )
        fresh_count = fresh_result.scalar()
        assert fresh_count > initial_count, "View should have more rows after refresh"
        ...
        assert refreshed_work_count == 1, "New work should be in refreshed materialized view"
    ...
    async def test_search_view_document_weights(
        ...
        assert work_ids_by_rank[0] == work1.id, "Work with search term in title should rank highest"
        ...
        for other_rank in other_ranks:
            assert title_rank > other_rank, "Title match should rank higher than subtitle/author matches"
    ...
    async def test_search_view_performance_with_large_dataset(
        ...
        assert refresh_time < 5.0, f"Materialized view refresh took too long: {refresh_time}s"
