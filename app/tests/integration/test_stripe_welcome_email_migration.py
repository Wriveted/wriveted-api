...
from app.models.event_outbox import EventOutbox, EventStatus, EventPriority
...
def test_stripe_welcome_email_creates_outbox_event(session):
    ...
    with patch('app.services.stripe_events.StripeSubscription') as mock_subscription, \
        ...
        subscription_mock.__getitem__.return_value = {"data": [{"price": {"id": "price_test_123"}}]}
        ...
        assert email_event.destination == "sendgrid:transactional"  # Email type appended to destination
        assert email_event.priority.lower() == EventPriority.CRITICAL.value.lower()  # TRANSACTIONAL emails are CRITICAL (case insensitive)
        ...
        payload = email_event.payload if isinstance(email_event.payload, dict) else json.loads(email_event.payload)
...
def test_stripe_welcome_email_not_sent_for_non_parent(session):
    ...
    with patch('app.services.stripe_events.StripeSubscription') as mock_subscription, \
        ...
        subscription_mock.current_period_end = 1672531200
        subscription_mock.__getitem__.return_value = {"data": [{"price": {"id": "price_test_456"}}]}
        mock_subscription.retrieve.return_value = subscription_mock
        ...
        assert final_email_count == initial_email_count, "Welcome email was sent for non-parent user"
...
def test_stripe_welcome_email_not_sent_without_customer_email(session):
    ...
    with patch('app.services.stripe_events.StripeSubscription') as mock_subscription, \
        ...
        subscription_mock.current_period_end = 1672531200
        subscription_mock.__getitem__.return_value = {"data": [{"price": {"id": "price_test_789"}}]}
        mock_subscription.retrieve.return_value = subscription_mock
        ...
        assert final_email_count == initial_email_count, "Welcome email was sent without customer email"