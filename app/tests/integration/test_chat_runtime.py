...
@pytest.mark.asyncio
async def test_message_node_processing(db_session, test_user):
    ...
    # Get initial node
    result = await chat_runtime.get_initial_node(db_session, flow.id, session)
...
@pytest.mark.asyncio
async def test_question_node_processing(db_session, test_user):
    ...
    db_session.add(question_content)
    ...
    response = await chat_runtime.process_interaction(
        db_session, session, user_input="John Doe", input_type="text"
    )
    ...
    updated_session = await chat_repo.get_session_by_token(
        db_session, session_token="test_token_456"
    )
    assert updated_session.state["name"] == "John Doe"


@pytest.mark.asyncio
async def test_condition_node_processing(db_session, test_user):
    ...
    db_session.add(adult_connection)
    ...
    db_session.add(minor_connection)
    ...
    result2 = await chat_runtime.get_initial_node(db_session, flow.id, session2)

    assert result2["messages"][0]["content"]["text"] == "Welcome, young user!"


@pytest.mark.asyncio
async def test_session_concurrency_control(db_session, test_user):
...
@pytest.mark.asyncio
async def test_session_history_tracking(db_session, test_user):
    ...
    session = await chat_runtime.start_session(
        db_session, flow_id=flow.id, user_id=test_user.id, session_token="history_test"
    )

    await chat_runtime.get_initial_node(db_session, flow.id, session)

    # Check history
    history = await chat_repo.get_session_history(db_session, session_id=session.id)
