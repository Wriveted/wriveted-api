...
class CRUDBase(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    def get_or_404(self, db: Session, id: Any) -> ModelType:
        ...
        if thing is None:
            raise HTTPException(status_code=404, detail=f"Resource {self.model.__name__} with id {id} not found.")
        return thing
    ...
    def get_all(self, db: Session, *, skip: int = 0, limit: int = 100, order_by=None) -> List[ModelType]:
    ...
    def get_multi(
            self, db: Session, ids: List[Any], *, skip: int = 0, limit: int = 100, order_by=None
    ) -> List[ModelType]:
