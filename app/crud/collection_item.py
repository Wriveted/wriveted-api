...
class CRUDCollectionItem(CRUDBase[CollectionItem, Any, Any]):
    ...
    async def create_in_bulk(self, session: Session, school_id, collection_items: list[CollectionItem]):
        ...
        previous_count = session.execute(select(func.count(CollectionItem.id)).where(CollectionItem.school_id == school_id)).scalar_one()

        stmt = insert(CollectionItem).on_conflict_do_nothing()
        session.execute(stmt, collection_items)
        session.commit()

        new_count = session.execute(select(func.count(CollectionItem.id)).where(CollectionItem.school_id == school_id)).scalar_one()
