...
def build_morse_flow_v2(theme_id: Optional[str]) -> Dict[str, Any]:
    ...
    nodes = [
        ...
        condition_node(
            "morse_decode_check",
            conditions=[
                {"if": "temp.morse_answer == 'HELLO'", "then": "$0"},
                {"if": "temp.morse_answer == 'hello'", "then": "$0"},
                {"if": "temp.morse_answer == 'Hello'", "then": "$0"},
            ],
            default_path="$1",
...
def build_caesar_flow_v2(theme_id: Optional[str]) -> Dict[str, Any]:
    ...
    nodes = [
        ...
        condition_node(
            "caesar_check",
            conditions=[{"if": "temp.caesar_answer == 'j'", "then": "$0"}],
            default_path="$1",
            x=1500,
            y=0,
        ),
...
def build_atbash_flow_v2(theme_id: Optional[str]) -> Dict[str, Any]:
    ...
    nodes = [
        ...
        condition_node(
            "atbash_check",
            conditions=[{"if": "temp.atbash_answer == 'n'", "then": "$0"}],
            default_path="$1",
            x=1200,
            y=0,
        ),
...
def build_hub_flow_v2(
    ...
    nodes = [
        ...
        condition_node(
            ...
            conditions=[
                {
                    "if": "temp.mission_choice == 'morse'",
                    "then": "$0",
                },
                {
                    "if": "temp.mission_choice == 'caesar'",
                    "then": "$1",
                },
            ],
        ...
        condition_node(
            "hq_loop_check",
            conditions=[{"if": "temp.another == 'yes'", "then": "$0"}],
            default_path="$1",
            x=2400,
            y=0,
        ),
