...
docker compose -f docker-compose.yml run --rm --entrypoint python api app/db/check_can_connect_to_db.py
docker compose -f docker-compose.yml run --rm --entrypoint bash api /app/scripts/start-tests.sh "$@"
