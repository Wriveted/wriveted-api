...
from app.models import BookList, CollectionItem, Edition, Hue, LabelSet, LabelSetHue, ReadingAbility, Work
...
for i, line in enumerate(huey_picks_data):
    ...
    editions = session.execute(crud.edition.get_multi_query(db=session, ids=isbns)).scalars().all()

    for edition in editions:
        if edition is not None and edition.work is not None and edition.work not in works_for_booklist:
...
booklist_orm = BookList(
    name="Huey's Picks",
    type=ListType.HUEY_LIST,
    info={},
    works=works_for_booklist
)
