...
print(
    "Adding a tightly coupled selection of ISBNs to db via api, to test that relationships are established correctly"
)
print("Connecting")
    ...
    for i in range(1, 10):
        ...
        if f"CNF{i}" in book:
            ...
            if book[f"CR{i}"] in ["A01", "A02"]:
                ...
                illustrators.append(
                    {"first_name": book[f"ICFN{i}"], "last_name": book[f"ICKN{i}"]}
                )

        # look for genres
        if f"BISACC{i}" in book:
            genres.append(
                {"bisac_code": book[f"BISACC{i}"], "name": book[f"BISACT{i}"]}
            )
